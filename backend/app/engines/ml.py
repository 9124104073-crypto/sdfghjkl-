...
* The model is a gradient-boosted regressor fitted on the 40-locality
  demonstration dataset, learning to reproduce that dataset's published AI
  score from the site's measured attributes.
* Forty rows is far too few to generalise. The model is a demonstration of
  the ML + SHAP pathway, not a validated predictor, and its `data_status` is
  always `ai_generated` with that caveat attached.
* SHAP values explain what the model did. They do not validate the score.
"""
...
class SiteScoreModel:
    ...
    def _fit_unlocked(self, sites: list[Site]) -> None:
        ...
        labelled = [s for s in sites if s.dataset_ai_score is not None]
        if len(labelled) < MIN_TRAINING_ROWS:
            raise MlUnavailable(
                f"Only {len(labelled)} labelled sites available; "
                f"at least {MIN_TRAINING_ROWS} are needed to fit a model."
            )

        X = np.array([[site_features(s)[f] for f in FEATURES] for s in labelled], dtype=float)
        y = np.array([float(s.dataset_ai_score) for s in labelled], dtype=float)

        # Shallow trees and strong regularisation: with 40 rows anything
        # deeper memorises the dataset instead of learning from it.
        model = XGBRegressor(
            n_estimators=220,
            max_depth=2,
            learning_rate=0.06,
        ...
        try:
            r2 = float(cross_val_score(model, X, y, cv=folds, scoring="r2").mean())
            mae = float(
                -cross_val_score(model, X, y, cv=folds, scoring="neg_mean_absolute_error").mean()
            )
        except Exception:
            r2, mae = None, None
        ...
        self._metrics = ModelMetrics(
            ...
            r2_cv=r2,
            mae_cv=mae,
            algorithm="XGBRegressor (depth 2, 220 trees)",
        )
        log.info("ML site model fitted on %d rows (cv R2=%s)", len(labelled), r2)

    def predict(self, site: Site, sites: list[Site]) -> MlPrediction:
        """Score one site and explain the prediction with SHAP."""
        self.fit(sites)
        ...
        contributions = [
            {
                ...
                "shap_value": round(float(values[i]), 3),
        ...
        prediction.notes = [
            "Predicted by a gradient-boosted model fitted on the 40-locality demonstration "
            "dataset; SHAP values explain the model's own reasoning, not real-world suitability.",
            "Forty training rows cannot support a generalisable model. This demonstrates the "
            "ML and SHAP pathway — the deterministic MCDA engine remains the default scorer.",
            "SHAP contributions are additive: base value plus all contributions equals the "
            "prediction.",
        ]
        return prediction

    def importances(self, sites: list[Site]) -> dict[str, Any]:
        """Global feature importance, averaged over |SHAP| across all sites."""
        self.fit(sites)
        if self._model is None:
            raise MlUnavailable(self._failed or "Model could not be fitted.")
        ...
        return {
            ...
            "notes": [
                "Mean absolute SHAP value across the training set — how much each attribute "
                "moves the model's prediction on average.",
                "Trained on demonstration data; indicative of the model, not of Chennai.",
            ],
        }
